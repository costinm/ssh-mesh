...
use std::{convert::Infallible, env, net::SocketAddr, sync::Arc, collections::HashMap, path::PathBuf};
...
use crate::mesh::ca::{CA, CertSignRequest};

/// HTTP request for certificate signing
#[derive(Debug, Deserialize, Serialize)]
struct HttpCertSignRequest {
    pub fqdn: String,
    pub sans: Vec<String>,
...
type HandlerFn = Arc<dyn Fn(Request<Incoming>) -> Pin<Box<dyn Future<Output = Result<Response<Full<Bytes>>, Infallible>> + Send>> + Send + Sync>;
...
impl H2Server {
    ...
    fn register_default_handlers(&mut self) {
        ...
        self.add_handler(
            "/_ssh".to_string(),
            Arc::new(|req| Box::pin(crate::mesh::ssh::handle_ssh_request(req)))
        );

        // Register certificate signing handler
        self.add_handler(
            "/_cert_sign".to_string(),
            Arc::new(|req| Box::pin(handle_cert_sign_request(req)))
        );
    }
    ...
    async fn handle_request(
        ...
        match handler_opt {
            ...
            None => handle_default_request(req).await,
        }
    }
        ...
        loop {
            ...
            tokio::task::spawn(async move {
                let service = service_fn(move |req| {
                    ...
                    async move {
                        H2Server::handle_request(req, handlers).await
                    }
                });
...
async fn handle_default_request(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, Infallible> {
...
/// HTTP handler for certificate signing requests
async fn handle_cert_sign_request(req: Request<Incoming>) -> Result<Response<Full<Bytes>>, Infallible> {
    info!("Received certificate signing request: {} {}", req.method(), req.uri());
    ...
    let body_bytes = match collected {
        ...
        Err(_) => {
            let response = Response::builder()
                .status(400)
                .body(Full::new(Bytes::from("Error reading request body")))
                .unwrap();
            return Ok(response);
        }
    };
    ...
    let http_csr: HttpCertSignRequest = match serde_json::from_slice(&body_bytes) {
        Ok(csr) => csr,
        Err(e) => {
            let response = Response::builder()
                .status(400)
    ...
    // Convert to internal CertSignRequest
    let csr = CertSignRequest {
        fqdn: http_csr.fqdn,
        sans: http_csr.sans,
        public_key: http_csr.public_key,
    };
    ...
    // Create CA instance and sign the certificate
    let mut ca = match CA::new(base_dir) {
        Ok(ca) => ca,
        Err(e) => {
            let response = Response::builder()
                .status(500)
                .body(Full::new(Bytes::from(format!("Error creating CA: {}", e))))
                .unwrap();
            return Ok(response);
        }
    };
    ...
    if ca.certificate.is_none() {
        if let Err(e) = ca.create_ca_certificate("DMesh CA") {
            let response = Response::builder()
                .status(500)
                .body(Full::new(Bytes::from(format!("Error creating CA certificate: {}", e))))
                .unwrap();
            return Ok(response);
        }
    }
    ...
    let cert_pem = match ca.sign_certificate(&csr) {
        Ok(cert) => cert,
        Err(e) => {
            let response = Response::builder()
                .status(500)
                .body(Full::new(Bytes::from(format!("Error signing certificate: {}", e))))
                .unwrap();
            return Ok(response);
        }
    };
    ...
    let response = Response::builder()
        ...
        .body(Full::new(Bytes::from(cert_pem)))
        .unwrap();
