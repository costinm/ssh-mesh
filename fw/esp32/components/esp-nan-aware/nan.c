...
static void handleDAF(wifi_pkt_rx_ctrl_t *rx_ctrl, NanPeer *peer, uint8_t *frame, uint32_t len, unsigned long now2) {
    ...
    if (aware.dump) {
        ESP_LOGI(TAG, "NANActionFrame len=%" PRIu32 " rssi=%d rate=%d mcs=%d ch=%d from=%2x:%2x:%2x:%2x:%2x:%2x",
                 len, rx_ctrl->rssi, (int)rx_ctrl->rate,
                 (int)rx_ctrl->mcs, (int)rx_ctrl->channel,
...
static void sniffer_cb1(void *buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (frame[24] != 4 || // public action
        ...
        ESP_LOG_BUFFER_HEXDUMP("nan-unknown", frame, len, ESP_LOG_INFO);
        ESP_LOGI(TAG, "NotNAN len=%" PRIu32 " rssi=%d rate=%d mcs=%d ch=%d %x %x %x",
                 len, rx_ctrl->rssi, (int)rx_ctrl->rate,
                 (int)rx_ctrl->mcs, (int)rx_ctrl->channel,
    ...
    aware.nanPackets++;
    if (frame[29] != 19) { // 0x13
        ESP_LOGI(TAG, "NotSDF len=%" PRIu32 " rssi=%d rate=%d mcs=%d ch=%d %x %x %x",
                 len, rx_ctrl->rssi, (int)rx_ctrl->rate,
                 (int)rx_ctrl->mcs, (int)rx_ctrl->channel,
                 frame[24], frame[25], frame[26]);
        return;
