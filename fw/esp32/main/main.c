...
#error Required PM_ENABLE
    ...
    if (err == ESP_OK && light_sleep > 0 ) {
        ESP_LOGI(TAG, "STARTUP: lsleep=%d", (int)light_sleep);
        // Configure dynamic frequency scaling:
        // maximum and minimum frequencies are set in sdkconfig,
        // automatic light sleep is enabled if tickless idle support is enabled.
        int xtal_freq = esp_clk_xtal_freq() / 1000000;
    ...
    if (ssid_len > 0) {
        ESP_LOGI(TAG, "SSID %s %s %d", ssid, psk, (int)mode);
    }
    if (mode == 4) {
        nanStart();
    } else {
    ...
    ESP_LOGI("l3dmesh", "Current time: RTC:%s RST:%lld FB:%lld wc:%d", strftime_buf,
            (long long)(currentTimeMicro() - resetTime),
            (long long)(currentTimeMicro() - firstBoot), wcause);
