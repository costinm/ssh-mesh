...
const MESH_INIT_PROXY = new URL('../../proxy/jsonrpc/mesh-init', window.location.href).toString();
...
// Paths are interpolated into legacy single-quoted inline handlers. Encode
// them for both the HTML attribute and its JavaScript string literal.
function escapeJsStringForHtmlAttribute(value) {
    return String(value)
        .replace(/&/g, '&amp;')
        .replace(/\\/g, '\\\\')
        .replace(/'/g, "\\'")
