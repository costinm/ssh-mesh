...
impl PyMeshNode {
    ...
    fn start(&mut self, py: Python<'_>, ssh_port: u16, http_port: u16) -> PyResult<()> {
        ...
        let handle = crate::mesh_common::start_mesh(
            &self.base_dir, ssh_port as i32, http_port as i32,
        )
    ...
    fn add_remote_forward(&self, py: Python<'_>, conn_id: u64, remote_port: u16, local_host: String, local_port: u16) -> PyResult<u32> {
        ...
        let res = py.allow_threads(|| {
            crate::mesh_common::mesh_add_remote_forward(handle, conn_id, remote_port, &local_host, local_port)
        });
    ...
    fn exec(&self, py: Python<'_>, conn_id: u64, command: String) -> PyResult<String> {
        ...
        let res = py.allow_threads(|| {
            crate::mesh_common::mesh_exec(handle, conn_id, &command)
        });
    ...
    fn open_stream(&self, py: Python<'_>, conn_id: u64, host: String, port: u16) -> PyResult<PyMeshStream> {
        ...
        let res = py.allow_threads(|| {
            crate::mesh_common::mesh_open_stream(handle, conn_id, &host, port)
        });
