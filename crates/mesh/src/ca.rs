use super::http::{H2Server, HandlerFn};
...
impl CA {
    ...
    pub fn sign_certificate(&self, csr: &CertSignRequest) -> Result<String> {
        let ca_cert = self
            .certificate
            .as_ref()
            .context("CA certificate not initialized")?;
        ...
        if !csr.sans.is_empty() {
            ...
            let san_extension =
                san_extension.build(&cert_builder.x509v3_context(Some(ca_cert), None))?;
    ...
    /// Register the certificate signing handler with H2Server
    pub fn register_handler_with_server(self: Arc<Self>, h2_server: &mut H2Server) {
        let handler = self.create_cert_handler();
        h2_server.add_handler("/_cert_sign".to_string(), handler);
    }

    /// Create a handler function for certificate signing requests
    fn create_cert_handler(self: Arc<Self>) -> HandlerFn {
        use std::convert::Infallible;

        Arc::new(
            move |req: Request<Incoming>| -> Pin<
            ...
            > {
                ...
                Box::pin(async move {
                    ...
                    let collected: Result<http_body_util::Collected<Bytes>, hyper::Error> =
                    ...
                    // Check if CA certificate exists (error if it doesn't)
                        ...
                        Err(_) => {
                            let response = Response::builder()
                                ...
                                .body(Full::new(Bytes::from(
                                    "Error signing certificate: CA certificate not initialized",
                                )))
                                .unwrap();
                            return Ok(response);
                        }
