...
| `esp.stability.start` | `source: string = "lora1"`, `expected: comma-list \| null`, `interval_sec: integer = 120`, `wait_sec: integer = 12`, `cycles: integer \| null` | Starts a background discovery/ping runner through the already-managed `source` UDS forward. It never opens a physical TTY. Each cycle sends `mode ping=true` on LoRa, raw action, and raw-NAN; raw-NAN queues the ping for the next Discovery Window when Wi-Fi is off. Observed LoRa pongs record sender address, uptime, RSSI/SNR, and responder raw-NAN counters. The cycle-level `nan` object captures source before/after counters and deltas; `response_observed=true` means the source received at least one raw-NAN pong in that cycle. |
| `esp.stability.status` | none | Returns the running state, configured source/targets, completed-cycle count, and the last cycle's expected, observed, and missing device roles. |
