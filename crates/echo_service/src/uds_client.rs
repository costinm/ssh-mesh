// UDS (Unix Domain Socket) echo client
// This client connects to the UDS echo service and performs benchmarking
...
pub fn run_client(args: &Args) -> Result<()> {
    ...
    println!("Starting UDS echo client...");
...
fn run_interactive(args: &Args) -> Result<()> {
    ...
    println!("Connected to UDS echo server");
    ...
    println!("Sending test messages to UDS echo service:");
...
fn run_benchmark(args: &Args) -> Result<()> {
    let mut stream = UnixStream::connect(&args.socket)?;
    println!("Connected to UDS echo server for benchmarking");
    ...
    println!(
        ...
        total_bytes as f64 / total_seconds
    );
    println!(
        "Throughput: {:.2} Mbps",
        (total_bytes as f64 * 8.0) / total_seconds / 1_000_000.0
    );
    println!(
        "Average latency: {:.6} sec/call",
        total_seconds / args.iterations as f64
    );
    println!(
        "Average latency: {:.3} µs/call",
        total_seconds * 1_000_000.0 / args.iterations as f64
    );
    println!(
        "Requests per second: {:.2}",
        args.iterations as f64 / total_seconds
    );
